...
function getDefaultTỷ LệFactor() {
...
let quality = 1;
let isThấpChất Lượng = false;
let isBình ThườngChất Lượng = true;
let isCaoChất Lượng = false;
...
// Toàn Màn Hình helpers, using Fscreen for prefixes.
...
function isToàn Màn Hình() {
  return !!fscreen.fullscreenElement;
}

// Attempt to toggle fullscreen mode.
function toggleToàn Màn Hình() {
  if (fullscreenEnabled()) {
    if (isToàn Màn Hình()) {
      fscreen.exitToàn Màn Hình();
    } else {
      fscreen.requestToàn Màn Hình(document.documentElement);
    }
  }
}
...
fscreen.addEventListener('fullscreenchange', () => {
  store.setState({ fullscreen: isToàn Màn Hình() });
});
...
const store = {
  ...
  state: {
    ...
    soundEnabled: false,
    menuOpen: false,
    openTrợ GiúpTopic: null,
    fullscreen: isToàn Màn Hình(),
    ...
    config: {
      ...
      size: IS_DESKTOP
        ? '3' // Desktop default
        : IS_HEADER 
          ? '1.2' // Profile tiêu đề default (doesn't need to be an int)
          : '2', // Mobile default
      autoKhởi Động: true,
      finale: false,
      ...
      longExposure: false,
      scaleFactor: getDefaultTỷ LệFactor()
    }
  },
...
function toggleTạm Dừng(toggle) {
...
function toggleÂm Thanh(toggle) {
  if (typeof toggle === 'boolean') {
...
function configDidUpdate() {
  ...
  isThấpChất Lượng = quality === QUALITY_LOW;
  isBình ThườngChất Lượng = quality === QUALITY_NORMAL;
  isCaoChất Lượng = quality === QUALITY_HIGH;
...
const canPhátÂm ThanhSelector = (state=store.state) => isRunning(state) && soundEnabledSelector(state);
...
const helpContent = {
  shellType: {
    tiêu đề: 'Loại Pháo Hoa',
    nội dung: 'The type of firework that will be launched. Select "Ngẫu Nhiên" for a nice assortment!'
  },
  shellSize: {
    tiêu đề: 'Kích Thước Pháo',
    nội dung: 'The size of the fireworks. Modeled after real firework shell sizes, larger shells have bigger bursts with more stars, and sometimes more complex effects. However, larger shells also require more processing power and may cause lag.'
  },
  quality: {
    tiêu đề: 'Chất Lượng',
    nội dung: 'Overall graphics quality. If the animation is not running smoothly, try lowering the quality. Cao quality greatly increases the amount of sparks rendered and may cause lag.'
  },
  skyLighting: {
    tiêu đề: 'Ánh Sáng Bầu Trời',
    nội dung: 'Illuminates the background as fireworks explode. If the background looks too bright on your screen, try setting it to "Mờ" or "Không Có".'
  },
  scaleFactor: {
    tiêu đề: 'Tỷ Lệ',
    nội dung: 'Allows scaling the size of all fireworks, essentially moving you closer or farther away. For larger shell sizes, it can be convenient to decrease the scale a bit, especially on phones or tablets.'
  },
  autoKhởi Động: {
    tiêu đề: 'Tự Động Bắn',
    nội dung: 'Khởi Độnges sequences of fireworks automatically. Sit back and enjoy the show, or disable to have full control.'
  },
  finaleMode: {
    tiêu đề: 'Chế Độ Cao Trào',
    nội dung: 'Khởi Độnges intense bursts of fireworks. May cause lag. Requires "Tự Động Bắn" to be enabled.'
  },
  hideControls: {
    tiêu đề: 'Ẩn Điều Khiển',
    nội dung: 'Hides the translucent controls along the top of the screen. Useful for screenshots, or just a more seamless experience. While hidden, you can still tap the top-right corner to re-open this menu.'
  },
  fullscreen: {
    tiêu đề: 'Toàn Màn Hình',
    nội dung: 'Chuyển Đổis fullscreen mode.'
  },
  longExposure: {
    tiêu đề: 'Mở Cửa Chớp',
    nội dung: 'Experimental effect that preserves long streaks of light, similar to leaving a camera shutter open.'
  }
};

const nodeKeyToTrợ GiúpKey = {
  shellTypeLabel: 'Loại pháo',
  shellSizeLabel: 'shellSize',
  qualityLabel: 'quality',
  skyLightingLabel: 'skyLighting',
  scaleFactorLabel: 'scaleFactor',
  autoKhởi ĐộngLabel: 'autoKhởi Động',
...
const appNodes = {
  ...
  autoKhởi Động: '.auto-launch',
  autoKhởi ĐộngLabel: '.auto-launch-label',
  ...
  helpModalHeader: '.help-modal__tiêu đề',
  helpModalBody: '.help-modal__nội dung',
  helpModalĐóngBtn: '.help-modal__close-btn'
};
...
function renderApp(state) {
  ...
  appNodes.finaleModeFormOption.style.opacity = state.config.autoKhởi Động ? 1 : 0.32;
  
  appNodes.quality.value = state.config.quality;
  appNodes.shellType.value = state.config.shell;
  appNodes.shellSize.value = state.config.size;
  appNodes.autoKhởi Động.checked = state.config.autoKhởi Động;
  ...
  appNodes.menuInnerWrap.style.opacity = state.openTrợ GiúpTopic ? 0.12 : 1;
  appNodes.helpModal.classList.toggle('active', !!state.openTrợ GiúpTopic);
  if (state.openTrợ GiúpTopic) {
    const { tiêu đề, nội dung } = helpContent[state.openTrợ GiúpTopic];
    appNodes.helpModalHeader.textContent = tiêu đề;
    appNodes.helpModalBody.textContent = nội dung;
  }
}
...
function handleStateChange(state, prevState) {
  const canPhátÂm Thanh = canPhátÂm ThanhSelector(state);
  const canPhátÂm ThanhPrev = canPhátÂm ThanhSelector(prevState);
  
  if (canPhátÂm Thanh !== canPhátÂm ThanhPrev) {
    if (canPhátÂm Thanh) {
...
function getConfigFromDOM() {
  return {
    ...
    autoKhởi Động: appNodes.autoKhởi Động.checked,
...
appNodes.autoKhởi Động.addEventListener('click', () => setTimeout(updateConfig, 0));
...
appNodes.fullscreen.addEventListener('click', () => setTimeout(toggleToàn Màn Hình, 0));
...
Object.keys(nodeKeyToTrợ GiúpKey).forEach(nodeKey => {
  const helpKey = nodeKeyToTrợ GiúpKey[nodeKey];
  appNodes[nodeKey].addEventListener('click', () => {
    store.setState({ openTrợ GiúpTopic: helpKey });
  });
});

appNodes.helpModalĐóngBtn.addEventListener('click', () => {
  store.setState({ openTrợ GiúpTopic: null });
});
...
const crysanthemumShell = (size=1) => {
  ...
  if (isThấpChất Lượng) starDensity *= 0.8;
  if (isCaoChất Lượng) starDensity = 1.2;
  return {
    shellSize: size,
...
const crackleShell = (size=1) => {
  ...
  return {
    ...
    starDensity: isThấpChất Lượng ? 0.65 : 1,
...
function randomShell(size) {
  // Special selection for codepen tiêu đề.
  if (IS_HEADER) return randomFastShell()(size);
  // Bình Thường operation
...
// Note this is only random when "Ngẫu Nhiên" shell is selected in config.
// Also, this does not create the shell, only returns the factory function.
const fastShellBlacklist = ['Falling Leaves', 'Floral', 'Willow'];
function randomFastShell() {
  const isNgẫu Nhiên = shellNameSelector() === 'Ngẫu Nhiên';
  let shellName = isNgẫu Nhiên ? randomShellName() : shellNameSelector();
  if (isNgẫu Nhiên) {
...
function init() {
  // Remove đang tải state
  document.querySelector('.đang tải-init').remove();
...
function getNgẫu NhiênShellPositionH() {
  return fitShellPositionInBoundsH(Math.random());
}

function getNgẫu NhiênShellPositionV() {
  return fitShellPositionInBoundsV(Math.random());
}

function getNgẫu NhiênShellSize() {
...
// Khởi Độnges a shell from a user pointer event, based on state.config
function launchShellFromConfig(event) {
  ...
  shell.launch(
    event ? event.x / w : getNgẫu NhiênShellPositionH(),
    event ? 1 - event.y / h : getNgẫu NhiênShellPositionV()
  );
}
...
function seqNgẫu NhiênShell() {
  const size = getNgẫu NhiênShellSize();
...
function seqNgẫu NhiênFastShell() {
...
function seqTwoNgẫu Nhiên() {
  const size1 = getNgẫu NhiênShellSize();
  const size2 = getNgẫu NhiênShellSize();
...
function updateGlobals(timeStep, lag) {
  ...
  // auto launch shells
  if (store.state.config.autoKhởi Động) {
    autoKhởi ĐộngTime -= timeStep;
    if (autoKhởi ĐộngTime <= 0) {
      autoKhởi ĐộngTime = startSequence() * 1.25;
    }
  }
}
...
function render(speed) {
  ...
  trailsCtx.lineCap = isThấpChất Lượng ? 'square' : 'round';
...
function colorSky(speed) {
  ...
  // Tỷ Lệ all color components to a max of `maxSkySaturation`, and apply intensity.
...
// Trợ Giúper used to semi-randomly spread particles over an arc
...
// Trợ Giúper used to create a spherical burst of particles
function createBurst(count, particleFactory, startAngle=0, arcLength=PI_2) {
  ...
  for (let i=0; i<=C_HALF; i++) {
    ...
    const maxNgẫu NhiênAngleOffset = angleInc * 0.33;
    
    for (let i=0; i<partsPerArc; i++) {
      const randomAngleOffset = Math.random() * maxNgẫu NhiênAngleOffset;
...
function floralEffect(star) {
  ...
  soundManager.playÂm Thanh('burstSmall');
} 
...
function crackleEffect(star) {
  const count = isCaoChất Lượng ? 32 : 16;
...
class Shell {
  ...
  launch(position, launchHeight) {
    ...
    if (isCaoChất Lượng) comet.sparkFreq = 8;
    comet.sparkLife = 320;
    ...
    // Ngẫu Nhiênly make comet "burn out" a bit early.
    ...
    soundManager.playÂm Thanh('lift');
  }
  ...
  burst(x, y) {
    ...
    let playedDeathÂm Thanh = false;
    
    if (this.crossette) onDeath = (star) => {
      if (!playedDeathÂm Thanh) {
        soundManager.playÂm Thanh('crackleSmall');
        playedDeathÂm Thanh = true;
      }
      crossetteEffect(star);
    }
    if (this.crackle) onDeath = (star) => {
      if (!playedDeathÂm Thanh) {
        soundManager.playÂm Thanh('crackle');
        playedDeathÂm Thanh = true;
      }
      crackleEffect(star);
    }
    ...
    if (this.glitter === 'light') {
      ...
      sparkFreq = 16;
      sparkSpeed = isCaoChất Lượng ? 1.65 : 1.5;
      sparkLife = 1400;
    ...
    // Phát sound, but only for "original" shell, the one that was launched.
    // We don't want multiple sounds from pistil or streamer "sub-shells".
    // This can be detected by the presence of a comet.
    if (this.comet) {
      // Tỷ Lệ explosion sound based on current shell size and selected (max) shell size.
      ...
      const soundTỷ Lệ = (1 - sizeDifferenceFromMaxSize / maxDiff) * 0.3 + 0.7;
      soundManager.playÂm Thanh('burst', soundTỷ Lệ);
    }
  }
}
...
// Trợ Giúper to generate objects for storing active particles.
...
const soundManager = {
  ...
  resumeAll() {
    // Phát a sound with no volume for iOS. This 'unlocks' the audio context when the user first enables sound.
    this.playÂm Thanh('lift', 0);
  ...
  /**
   * Phát a sound of `type`. Will randomly pick a file associated with type, and play it at the specified volume
   * and play speed, with a bit of random variance in play speed. This is all based on `sources` config.
   *
   * @param  {string} type - The type of sound to play.
   * @param  {?number} scale=1 - Value between 0 and 1 (values outside range will be clamped). Tỷ Lệs less than one
  ...
  playÂm Thanh(type, scale=1) {
    ...
    // *all* the way back.
    if (!canPhátÂm ThanhSelector() || simSpeed < 0.95) {
      return;
    }
    ...
    if (!source) {
      throw new Error(`Âm Thanh of type "${type}" doesn't exist.`);
    }
    
    const initialVolume = source.volume;
    const initialPhátbackRate = MyMath.random(
    ...
    // Phátback rate increases with scale. For this, we map the scale of 0-1 to a scale of 2-1.
    // So at a scale of 1, sound plays normally, but as scale approaches 0 speed approaches double.
    const scaledPhátbackRate = initialPhátbackRate * (2 - scale);
    ...
    bufferSource.playbackRate.value = scaledPhátbackRate;
...
function setLoadingStatus(status) {
  document.querySelector('.đang tải-init__status').textContent = status;
}

// CodePen profile tiêu đề doesn't need audio, just initialize.
if (IS_HEADER) {
  init();
} else {
